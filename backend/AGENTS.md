...
- Prisma/infrastructure error를 비즈니스 의미로 해석해야 할 경우 애플리케이션 에러로 변환한다.
...
- Supertest의 `response.body`처럼 외부 라이브러리에서 `any`가 들어오는 경계는 helper나 명시적 narrowing으로 처리한다.
...
- repository에서는 Prisma query에 집중한다.
- Prisma error를 HTTP status로 직접 해석하지 않는다.
- 비즈니스 의미가 필요한 Prisma error 변환은 service에서 수행한다.

## 6. TypeScript / ESM

- TypeScript + ESM 구조를 유지한다.
- 기존 path alias/import 규칙을 따른다.
- generated Prisma code의 import 형식을 수동으로 수정하지 않는다.
